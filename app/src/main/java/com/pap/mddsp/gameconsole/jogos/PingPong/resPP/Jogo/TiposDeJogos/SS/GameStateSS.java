...
public class GameStateSS {
    ...
    public GameStateSS(Context contextt)
    {
        context = contextt;
        bottomBatX = 0;
        bottomBatY = 0;
    ...
    public void veriCBB()
    {
        ...
        if(randomB > 98){
            float percDezW = screenWidth/10;
            float percDezH = screenHeight/10;

            if((ball.bot < screenHeight - percDezH) && (ball.topo > percDezH)){
                int Ymax = (int) screenHeight - (int)percDezH;
                int Ymin = (int)percDezH;
                int randomY = new Random().nextInt(Ymax - Ymin) + Ymin;

                int Xmax = (int) screenWidth - (int)percDezW;
                int Xmin = (int)percDezW;
                int randomX = new Random().nextInt(Xmax - Xmin) + Xmin;

                ball.mudarCC(randomX,randomY);
            }
        }
    }
